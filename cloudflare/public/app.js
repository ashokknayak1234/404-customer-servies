...
let signedIn = false, userRole = 'admin', booting = true, pendingRoute = 'overview', tickets = [], stats = null;
...
function resetSession() {
  signedIn = false; userRole = 'admin'; tickets = []; stats = null; highlightedId = null;
...
function renderRoute() {
  ...
  if (userRole === 'customer' && route === 'overview') {navigate('tickets'); return;}
  ...
  if (userRole === 'customer') {
    all('a[data-workspace-nav="overview"], #settings-open').forEach(el => el.hidden = true);
    $('.workspace-heading .eyebrow').textContent = 'YOUR SUPPORT TICKETS';
    $('#workspace-title').textContent = 'Your tickets.';
    $('#workspace-description').textContent = 'Track your requests and open issues.';
  } else {
    all('a[data-workspace-nav="overview"], #settings-open').forEach(el => el.hidden = false);
    $('.workspace-heading .eyebrow').textContent = 'YOUR SUPPORT WORKSPACE';
  }
...
async function refresh() {
  ...
  loadingDashboard = (async() => {
    try {
      const endpoint = userRole === 'customer' ? '/api/customer/dashboard' : '/api/admin/dashboard';
      ...
      $('#dashboard-error').hidden = true; renderTickets(); return true;
    } catch(error) {
...
function renderTickets() {
  ...
  for (const ticket of visible) {
    ...
    actions.append(escalate);card.append(actions);root.append(card);
  }
}
...
async function boot() {
  ...
  signedIn=session.status==='fulfilled';
  userRole=signedIn?session.value.role:'admin';
  $('#nav-auth').textContent=signedIn?'Sign out ↗':'Sign in ↗';booting=false;renderRoute();
