        ...
        try:
            ...
            if api:
                ...
                if request.url.path not in ("/health", "/api/auth/login"):
                    session_data = await request.state.db.session(
...
def create_app(env=None, store_factory=storage, classifier=classify_ticket):
    ...
    async def login(data: AdminLoginRequest, request: Request):
        ...
        customer_configured = setting(
            request.state.env,
            "CUSTOMER_PASSWORD",
            setting(request.state.env, "ITTRS_CUSTOMER_PASSWORD", "local-demo-only" if setting(request.state.env, "APP_ENV") == "local" else None),
        )
        customer_user = setting(
            request.state.env,
            "CUSTOMER_USER",
            setting(request.state.env, "ITTRS_CUSTOMER_USER", "customer"),
        )
        
        username_hash = hashed(text(data.username, 100))
        password_hash = hashed(text(data.password, 512))

        is_admin = secrets.compare_digest(username_hash, hashed(user)) and secrets.compare_digest(password_hash, hashed(configured))
        is_customer = customer_configured and secrets.compare_digest(username_hash, hashed(customer_user)) and secrets.compare_digest(password_hash, hashed(customer_configured))
        
        if not is_admin and not is_customer:
            raise HTTPException(401, "Invalid username or password.")

        role = "admin" if is_admin else "customer"
        assigned_username = user if is_admin else customer_user

        token, now = secrets.token_hex(32), int(time.time())
        await request.state.db.create_session(hashed(token), now + 28800, now, role, assigned_username)
    ...
    async def me(request: Request):
        ...
        if user_data:
            return {"username": user_data.get("username", "admin"), "role": user_data.get("role", "admin")}
        return {"username": setting(request.state.env, "ADMIN_USER", "admin"), "role": "admin"}
    ...
    @app.get("/api/customer/dashboard", response_model=CustomerDashboardResponse)
    async def customer_dashboard(request: Request):
        user_data = getattr(request.state, "user", None)
        if not user_data or user_data.get("role") != "customer":
            raise HTTPException(403, "Access denied.")
        return await request.state.db.customer_dashboard(user_data.get("username"))
