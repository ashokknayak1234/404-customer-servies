...
class LocalStore:
    def __init__(self, path):
        ...
        with self.connection() as db:
            db.executescript("""
                ...
                CREATE TABLE IF NOT EXISTS sessions (token_hash TEXT PRIMARY KEY, expires_at INTEGER NOT NULL, role TEXT NOT NULL DEFAULT 'admin', username TEXT);
    ...
    async def session(self, token_hash, now):
        with self.connection() as db:
            row = db.execute(
                "SELECT expires_at, role, username FROM sessions WHERE token_hash=? AND expires_at>?",
                (token_hash, now),
            ).fetchone()
            return dict(row) if row else None

    async def create_session(self, token_hash, expires, now, role="admin", username=None):
        with self.connection() as db:
            db.execute("DELETE FROM sessions WHERE expires_at <= ?", (now,))
            db.execute("INSERT INTO sessions (token_hash, expires_at, role, username) VALUES (?,?,?,?)", (token_hash, expires, role, username))
    ...
    async def customer_dashboard(self, username):
        with self.connection() as db:
            tickets = [
                dict(row)
                for row in db.execute(
                    "SELECT * FROM tickets WHERE customer_username=? ORDER BY created_at DESC LIMIT 100",
                    (username,)
                )
            ]
