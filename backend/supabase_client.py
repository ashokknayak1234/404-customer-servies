...
class SupabaseStore:
    ...
    async def session(self, token_hash, now):
        rows = await self.request(
            f"ittrs_sessions?token_hash=eq.{quote(token_hash)}&expires_at=gt.{now}&select=expires_at,role,username&limit=1"
        )
    ...
    async def create_session(self, token_hash, expires, now, role="admin", username=None):
        await self.request(
            ...
            "POST",
            {"p_hash": token_hash, "p_expires": expires, "p_now": now, "p_role": role, "p_username": username},
        )
    ...
    async def customer_dashboard(self, username):
        return await self.request("rpc/ittrs_customer_dashboard", "POST", {"p_username": username})
